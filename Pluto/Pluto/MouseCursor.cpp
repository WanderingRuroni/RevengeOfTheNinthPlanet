...
MouseCursor::MouseCursor( Game* game, ID3D11Device* d3dDevice, ID3D11DeviceContext* d3dContext, IDXGISwapChain* swapChain, ID3D11RenderTargetView* backBufferTarget ) : solidColorVS_( 0 ), solidColorPS_( 0 ), inputLayout_( 0 ),
...
GameSprite* MouseCursor::GetSprite( )
{
	return &sprite_;
}
...
{
	sprite_ = spriteSheet_->CreateSprite( collisionTemplate_, spriteWidth_, spriteHeight_, 0.0f, XMFLOAT2( 500.0f, 500.0f ) );
	sprite_.SetCurrentAnimation("cursor");
}
...
bool MouseCursor::LoadContent( )
{
    ...
    d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice_,
        "Assets/Images/Mouse.png", 0, 0, &colorMap_, 0 );
	...
	vector<XMFLOAT2> cursor;
	cursor.push_back( XMFLOAT2( 0.0f, 0.0f ) );

	spriteSheet_->AddAnimationToMap( "cursor" , cursor, -1 );
...
void MouseCursor::UnloadContent( )
{
...
void MouseCursor::Render( XMMATRIX* vpMatrix )
{
