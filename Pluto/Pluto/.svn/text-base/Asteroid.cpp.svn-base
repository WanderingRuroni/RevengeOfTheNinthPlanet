...
Asteroid::Asteroid(Game* game, ID3D11Device* d3dDevice, ID3D11DeviceContext* d3dContext, IDXGISwapChain* swapChain, ID3D11RenderTargetView* backBufferTarget ) : solidColorVS_( 0 ), solidColorPS_( 0 ), inputLayout_( 0 ),
...
list<GameSprite>* Asteroid::GetSprites()
{
	return &sprites_;
}
...
bool Asteroid::LoadContent()
{
    ...
    d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice_,
        "Assets/Images/head.png", 0, 0, &colorMap_, 0 );
...
void Asteroid::Update( float dt )
{
	...
	for( list<GameSprite>::iterator s = sprites_.begin( ); s != sprites_.end( ); s++ )
	{
		...
		else
		{
			...
			s->SetRotation( s->GetRotation( ) + 3.0f * dt );
		}
...
void Asteroid::Render( XMMATRIX* vpMatrix )
{
...
void Asteroid::CreateAsteroid( XMFLOAT2 position, float rotation, XMFLOAT2 scale )
{
	GameSprite sprite_ = spriteSheet_->CreateSprite( collisionTemplate_, spriteWidth_, spriteHeight_, 75.0f, position, 10, 1, rotation, scale );
